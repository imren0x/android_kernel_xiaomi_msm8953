...
ARCH=arm64 scripts/kconfig/merge_config.sh -O "out_$DEVICENAME" arch/arm64/configs/msm8953-perf_defconfig arch/arm64/configs/xiaomi/xiaomi.config arch/arm64/configs/xiaomi/daisy.config lineageos_xx_append /tmp/lineageos_xx_devicename
...
#	exit
#	fi
#ARCH=arm64 scripts/kconfig/merge_config.sh -O "out_$DEVICENAME" arch/arm64/configs/msm8953-perf_defconfig arch/arm64/configs/xiaomi/xiaomi.config arch/arm64/configs/xiaomi/"$DEVICENAME".config lineageos_xx_append /tmp/lineageos_xx_devicename
#fi


make -j12 ARCH=arm64 SUBARCH=arm64 O=out_$DEVICENAME \
        CC="ccache clang"\
	AS="clang" \
        AR="llvm-ar" \
	NM="llvm-nm" \
